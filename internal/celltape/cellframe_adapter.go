...
// InputFromCellFrame is the compatibility DTO adapter. New live paths should
// call Recorder.PrepareCellFrame instead, which avoids this intermediate copy.
// Call it before releasing frame. Commit still takes the separate host time.
func InputFromCellFrame(frame *cellframe.CellFrame, configEpoch uint64, config []byte, boundary BoundaryFlags) (Input, error) {
	if frame == nil || !frame.Valid() {
		return Input{}, errors.New("celltape: cellframe lease is not valid")
	}
	if frame.Columns() <= 0 || frame.Rows() <= 0 || uint64(frame.Columns()) > uint64(^uint32(0)) || uint64(frame.Rows()) > uint64(^uint32(0)) {
		return Input{}, errors.New("celltape: cellframe geometry is out of range")
	}
	if frame.SourcePTS() < 0 {
		return Input{}, errors.New("celltape: negative source PTS")
	}
	in := Input{
		GeometryEpoch: frame.GeometryEpoch(),
		ConfigEpoch:   configEpoch,
		Columns:       uint32(frame.Columns()),
	...
	for i := range in.Cells {
		cell, ok := frame.Cell(i)
		if !ok {
			return Input{}, fmt.Errorf("celltape: cellframe cell %d unavailable", i)
		}
		converted, err := CellFromPacked(cell.Packed())
		if err != nil {
			return Input{}, fmt.Errorf("celltape: cellframe cell %d: %w", i, err)
		}
		in.Cells[i] = converted
	}
	return in, nil
}
