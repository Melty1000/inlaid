...
For an ordinary compatibility test, use the normal Inlaid application; there is no first-run test, tester mode, or separate executable. Exercise the camera and outputs first, then select **Report** and confirm with **Create Report**. Inlaid writes one bounded JSON file under `support-reports`. Select **Open Folder** to open that location, inspect the JSON as text, and remove anything you do not want public. If you choose to submit the result, open the compatibility form yourself and attach that one JSON file. Inlaid does not create a ZIP, upload the report, or create an issue.
